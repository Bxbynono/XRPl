...
export default function AdminProfile() {
    ...
    const uploadUser = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post('http://localhost:3001/studentVault/v1/users', {
                name: name,
                email: email,
    ...
    const update = async (e) => {
        ...
        try {
            ...
            const decodedToken = jwt.decode(cookieValue);
            const userId = decodedToken.id;
            const response = await axios.patch(`http://localhost:3001/studentVault/v1/users/${userId}`, formData);
