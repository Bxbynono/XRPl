...
exports.login = async (req, res, next) => {
    try {
        ...
        if (!secret || !password) {
            return next(new AppError("Please provide the secret key and password", 400))
        }

        //check if the user exits && and password is correct
        const user = await User.findOne({secret}).select('+password')

        if (!user || !(await user.correctPassword(password, user.password))) {
            return next(new AppError('Incorrect secret key or password', 401))
        }
